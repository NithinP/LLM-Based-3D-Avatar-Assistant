...
public class AudioVisualizeManager : MonoBehaviour
{
    ...
    IEnumerator StartBeat()
    {
        ...
        while (beat)
        {
            ...
            {
                // Get the current position in the audio buffer
                int pos = Microphone.GetPosition(null);
                if (pos < 0 || pos >= audioSource.clip.samples)
                {
                    yield return null;
                    continue;
                }

                // Calculate the start position for sampling
                int startPos = pos - samplesArray.Length;
                if (startPos < 0) startPos += audioSource.clip.samples;

                // Get the audio data
                audioSource.clip.GetData(samplesArray, startPos);
